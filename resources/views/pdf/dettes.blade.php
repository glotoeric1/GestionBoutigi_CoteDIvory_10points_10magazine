...
<head>
    ...
    <title>Facture_créance_n°_{{ $clientId }}</title>
</head>
...
<body>
    ...
    <h4 style="text-align: center">Facture créance </h4>
    <p style="text-align: center">Fait le : {{ date('d-m-Y', strtotime($date_hr)) }} à
        {{ date('H:i:s', strtotime($date_hr)) }}
    </p>
    <p style="text-align: center">À payer le : {{ date('d-m-Y', strtotime($apayer)) }}</p>
    <table style=" width: 100%;" border="2">
        <tbody>
            <tr>
                <td colspan="4" style="text-align:center">Facture N° : {{ $clientId }} </td>
            </tr>
            <tr>
    ...
    <table style="width: 100%;" border="2">
        <tbody>
            ...
            @foreach (session('dettes') as $cart)
                <tr>
                    <td style="padding-left: 5px;">
                        {{ $cart['produit'] }}
                    </td>
    ...
    <h5 class="text-center">Arrête le present créance à la somme de {{ ucfirst($amountInWords) }}</h5>
