@php
    ...
    /*
    |--------------------------------------------------------------------------
    | SAFE DEFAULTS (never crash PDF)
    |--------------------------------------------------------------------------
    */

    $cc = " F CFA";
    $i = 1;
    $totalLignes = 0;   // Grand total of all line items (sum of "montant")
    ...
    $totalM = $totalM ?? 0;  // Total TTC from vente table
    ...
    */
    ...
    /*
    |--------------------------------------------------------------------------
    | FILTERED PRODUCT NAME (for the summary)
    |--------------------------------------------------------------------------
    */
    if (request()->filled('id_prod')) {
        // --- ADAPT THIS TO YOUR PRODUCT MODEL AND COLUMN ---
...
<p style="text-align:center;">
    Date : {{ Carbon::parse($date_hr)->format('d/m/Y H:i:s') }}
</p>
    ...
    <tbody>
        @forelse($carts as $vente)
            @foreach($vente->items as $item)
                @php
                    // --- Fetch the product name safely ---
                    // Replace Produit with your actual product model name,
                    // and nom/name with the correct column.
                    $prod = \App\Models\Stock::find($item->id_prod);
                    $prodName = $prod->libelle ?? $prod->libelle ?? 'Produit #' . $item->id_prod;

                    // Amount for this line is stored in the detail table
                    $montantLigne = $item->montant ?? 0;
                    $totalLignes += $montantLigne;   // accumulate grand total
                @endphp
...
<table width="100%" border="1" cellpadding="6" style="border-collapse:collapse;">
    ...
    {{-- ✅ GRAND TOTAL OF ALL LINE ITEMS (calculated entirely in the blade loop) --}}
    <tr>
    ...
    @if($operation === "VENTE")
        ...
        <tr>
            <td><b>Total après réduction</b></td>
            <td>{{ number_format(max(0, $totalLignes - $totalR), 0, ",", " ") }}{{ $cc }}</td>
        </tr>
