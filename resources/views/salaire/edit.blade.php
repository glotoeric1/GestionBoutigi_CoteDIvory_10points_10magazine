...
<div class="row">
    ...
    <div class="card card-primary">
      ...
      <!-- /.card-header -->
      <!-- form start -->
      <form action="{{ route("salaires.update", [$datas->id]) }}" method="post">
      @csrf
      @method("PUT")
        <div class="card-body">
              ...
              <select name="emp_id" id="emp_id" onblur="calculateBalance()" class="form-control @error('contact') is-invalid @enderror">
                ...
                @if (count($emps)>0)
                  @foreach ($emps as $data)
                    <option value="{{$data->id}}" {{($datas->emp_id==$data->id) ? "selected" : ''}}>{{$data->nom}}</option>
                  @endforeach
                @endif
              </select>
              @error("emp_id")
        ...
        <div class="row g-3">
          <div class="col-md-4">
            ...
            <div class="form-group">
              ...
              <input type="text" name="salaire" value="{{$datas->salaire}}" onblur="calculateBalance()" class="form-control @error('salaire') is-invalid @enderror" id="salaire" placeholder="" readonly>
              @error("salaire")
                  <span class="text-danger"> {{$message}}</span>
              @enderror
          </div>
        ...
        <div class="col-md-4">
          ...
          <div class="form-group">
            ...
            <input type="text" name="montantRecu" value="{{$datas->montantRecu}}" onblur="calculateBalance()" class="form-control @error('montantRecu') is-invalid @enderror" id="montantRecu" placeholder="" >
            @error("montantRecu")
                <span class="text-danger"> {{$message}}</span>
            @enderror
        </div>
      ...
      <div class="col-md-4">
        ...
        <div class="form-group">
          ...
          <input type="text" name="montantRestant" value="{{$datas->montantRestant}}" class="form-control @error('montantRestant') is-invalid @enderror" id="montantRestant" placeholder="" readonly >
          ...
          <div class="form-group">
            ...
            <select name="years" class="form-control @error('years') is-invalid @enderror" id="years" >
              ...
              @foreach ($years as $year)
                <option value="{{$year}}" {{($year==$datas->years) ? "selected" : ''}} >{{$year}}</option>
              @endforeach
          </select>
            @error("years")
                <span class="text-danger"> {{$message}}</span>
      ...
      <div class="col-md-4">
        ...
        <div class="form-group">
          ...
          <select name="mois" class="form-control @error('mois') is-invalid @enderror" id="mois" >
              <option value="">...</option>
              @if ($datas->mois)
              <option value="{{$datas->mois}}" selected>{{$datas->mois}}</option>
              @endif
    ...
    <div class="col-md-4">
      ...
      <div class="form-group">
        ...
        <input type="number" name="bonus" value="{{$datas->bonus}}" class="form-control @error('bonus') is-invalid @enderror" id="bonus" placeholder="" >
        @error("bonus")
            <span class="text-danger"> {{$message}}</span>
        @enderror
    </div>
